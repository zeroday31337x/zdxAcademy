...
export async function POST(req: Request) {
  ...
  if (!body.lessonId || !body.languageCode || !body.title || !body.bodyMarkdown) {
    return NextResponse.json({ error: "lessonId, languageCode, title and bodyMarkdown required" }, { status: 400 });
  }

  const run = await academyWorkerRpc("academy_worker_enqueue", {
    p_course_id: course.id,
    p_agent_type: "translator",
    p_input: {
      lessonId: body.lessonId,
      languageCode: body.languageCode,
      title: body.title,
      bodyMarkdown: body.bodyMarkdown,
      sourceLessonHash: body.sourceLessonHash || null,
      autoPipeline: false
    }
  });
